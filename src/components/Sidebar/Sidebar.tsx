...
const Sidebar = () => {
  ...
  return (
    <aside className='flex max-w-[238px] w-full flex-col h-full justify-between border-r border-secondary-light-grey'>
      <div className='flex flex-col gap-4 pr-6'>
        {sidebarLinks.map(item => (
          <Link href={item.link} key={item.name} className='flex w-full items-center gap-3'>
            ...
            <div className='text-primary-black text-lg'>{item.name}</div>
          </Link>
        ))}
      </div>
